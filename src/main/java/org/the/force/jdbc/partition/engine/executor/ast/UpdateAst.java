 ...
 */
public class UpdateAst extends AbstractExecutableAst {

    private static Log logger = LogFactory.getLog(UpdateAst.class);
    ...
    public UpdateAst(LogicDbConfig logicDbConfig, SQLUpdateStatement sqlStatement) throws Exception {
        super(logicDbConfig);
        this.sqlUpdateStatement = sqlStatement;
        exprSqlTable = new ExprConditionalSqlTable(logicDbConfig, (SQLExprTableSource) sqlUpdateStatement.getTableSource());
        new TableConditionParser(logicDbConfig, exprSqlTable, sqlUpdateStatement.getWhere());
        new SqlTableReferParser(logicDbConfig, sqlUpdateStatement, exprSqlTable);
